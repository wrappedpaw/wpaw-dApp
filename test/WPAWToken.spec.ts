  ...
  beforeEach(async () => {
	...
	describe('Swaps: PAW -> wPAW', () => {
		...
		it('Refuses to mint if the amount and uuid parameters are swapped', async () => {
			...
			const sig: Signature = await ReceiptsUtil.createReceipt(owner, user1.address, wPawToMint, uuid, await owner.getChainId());
			await expect(user1_interaction.mintWithReceipt(user1.address, uuid, wPawToMint, sig.v, sig.r, sig.s))
				.to.be.revertedWith("Signature invalid");
		});
		...
		it('Refuses to mint if the receipt was not signed by the owner', async () => {
			...
			const sig: Signature = await ReceiptsUtil.createReceipt(user1, user1.address, wPawToMint, uuid, await owner.getChainId());
		...
		it('Refuses to mint if using wrong chain ID', async () => {
			...
			const sig: Signature = await ReceiptsUtil.createReceipt(owner, user1.address, wPawToMint, uuid, 98765);
			await expect(user1_interaction.mintWithReceipt(user1.address, wPawToMint, uuid, sig.v, sig.r, sig.s))
				.to.be.revertedWith("Signature invalid");
		});
		...
		it('Mints wPAW if the receipt matches parameters', async () => {
			...
			expect(await token.isReceiptConsumed(user1.address, wPawToMint, uuid)).to.be.true;
		});
